...
nt_data_ptr     equ $80  ; 2 bytes; read pointer to name table data (low byte first)
ppu_addr_copy   equ $82  ; 2 bytes; copy of ppu_addr (low byte first)
...
-               sta ppu_data
                ...
                lda #%10000000          ; NMI to be enabled, name table 0 to be shown
                sta ppu_ctrl_copy
...
main_loop       bit run_main            ; wait until NMI routine has set flag
                ...
                ; (nt_data must be at $xx00)
                ;
                lda counter_lo          ; nt_data_ptr = counter_lo << 7
                lsr a
                sta nt_data_ptr+1
                lda #$00
                ror a
                sta nt_data_ptr+0
                ;
                lda counter_hi          ; nt_data_ptr += nt_data + counter_hi * $300
                asl a
                adc counter_hi          ; carry is always clear
                adc #>nt_data
                adc nt_data_ptr+1
                sta nt_data_ptr+1

                ldy #0                  ; copy 128 bytes (4 rows) of name table (video) data
-               lda (nt_data_ptr),y     ; to buffer
                sta vram_buffer,y       ; (NMI routine can read it faster from there)
                iny
                bpl -

                ; ppu_addr_copy = $2060 + (counter_hi & 1) * $0400 + counter_lo * $80
                lda counter_hi          ; high byte:
                and #%00000001          ; $20 | ((counter_hi & 1) << 2) | (counter_lo >> 1)
                asl a
                asl a
                asl a
                ora counter_lo
...
; --- Name table (video) data ---------------------------------------------------------------------
                ...
                align $100, $ff         ; to simplify address calculation

nt_data         ; IIRC, I copied this from some video on https://tasvideos.org
...
nmi             pha                     ; push A, X (note: not Y)
                ...
                lda ppu_addr_copy+1     ; set PPU address
                sta ppu_addr
                lda ppu_addr_copy+0
                sta ppu_addr
...
set_ppu_regs    lda #$00                ; clear ppu_scroll, update ppu_ctrl
                sta ppu_scroll
                sta ppu_scroll
                lda ppu_ctrl_copy
                sta ppu_ctrl
