...
; --- Start of PRG ROM ----------------------------------------------------------------------------
            ...
            pad $10000-95, $ff  ; note: set start address manually to as large as possible
...
-           bit ppustatus  ; wait for start of VBlank three times; also clear X
            ...
            jsr setppuaddr  ; tile 0: color #0 only (16 * 0x00)
            ldy #2          ; tile 1: color #2 only (8 * 0x00, 8 * 0xff)
--          ldx #24         ; tile 2: color #3 only (16 * 0xff)
-           sta ppudata     ; that is, write 24 * 0x00 and 24 * 0xff
            dex
            bne -
            lda #$ff
            dey
...
-           lda palette,y
            ...
            jsr setppuaddr  ; each stripe in name table is 6*32 = 192 bytes
            ldy #5          ; "6th stripe" actually clears attribute table 0 (and writes garbage
--          lda tiles,y     ; to start of name table 1, which is why we use vertical mirroring)
            ldx #(6*32)
-           sta ppudata
            dex
            bne -
